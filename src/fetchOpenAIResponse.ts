...
export const fetchOpenAIResponseExercises = async (
  ...
  const notesTexts = await Promise.all(notes.map((note) => app.vault.read(note)));

  const exercisesToGenerate: NoteExercisesToGenerate[] = notesTexts.map((text) => ({
    noteText: text,
    exercises: ["single-choice", "multiple-choice", "single-choice"],
  }));
